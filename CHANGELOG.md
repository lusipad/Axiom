...
## Unreleased — CNC algorithm comparison

- 新增应用层外部 XYZ 指令 A/B 比较、案例身份检查、不同运动时长支持、采样路径偏差与离散 V/A/J 诊断、最差位置和确定性报告。
- Python、CLI 与 HTTP 共用比较器；支持三文件或完整请求重放、UTF-8 BOM 和显式合成示例。
- 网页默认进入算法对比，原有阶段页面收进参考/实验入口；导入变化和失败立即清除过期报告。
- 近期路线收敛为真实算法版本与生产刀路回归，暂停 R5–R7 横向扩展和先行新内核重建。
- 修正 M5 power-sum 辅助上界：精确有理数复核输入 binary64 系数，防止最终单次 nextafter 未覆盖累计舍入误差。该修正不构成系数推导和完整连续证书的整体审计。
- 本次不宣称真实生产算法已接入或实机效果已验证；样例始终为 synthetic-example。
