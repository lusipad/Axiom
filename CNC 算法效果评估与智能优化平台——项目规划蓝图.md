...
> 文档类型：产品总纲与 Roadmap  
> 状态：Draft / 方向重整与契约闭合版 v0.17.0
> 当前阶段：R7-D Beckhoff TwinCAT 厂商验收合同片已实现 / 厂商运行时、现实验证、受控试验与设备写入 gate 保持 Open
> 更新日期：2026-08-13
...
R3–R7 的设备、物理、学习、受约束推荐与受控运行规范已经创建。R7-D 已选择 Beckhoff TwinCAT 3 / TF6100 并闭合 Profile、只读 verifier、DomainPack/API/UI 与发布合同；由于尚无经授权的真实 TwinCAT 环境，`vendorRuntimeStatus`、跨设备/工况 `realWorldGeneralizationStatus`、reality validation、deployment Shadow、Controlled Trial 与自动闭环仍保持 Open。
