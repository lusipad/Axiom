...
> 文档类型：产品总纲与 Roadmap  
> 状态：Draft / 方向重整与契约闭合版 v0.7
> 当前阶段：R0/R1 已闭合 + R2 Math F3
> 更新日期：2026-08-11
...
实现上，DomainPack 的可哈希描述符与进程内运行绑定分离；Core 只按 `domainPackId` 解析绑定并保存领域中立 Artifact envelope。任何五轴采样视图与有序点之间的转换都必须经过版本化 Artifact Adapter，产生新的内容标识并记录保留/丢弃语义。FiveAxis F0 以“契约与能力面板”进入网页；F1 则可以呈现已经实际求值的 M0–M2 几何、连续误差、标准对应和任务几何碰撞声明。Math F4 完成前，界面必须标出当前数学阶段和标准 Claim 范围，不能把 F1 的任务几何证据呈现为运动学、整机碰撞或设备可执行性结论。
...
端侧模型不是把训练代码直接搬到设备。R5 应把训练与部署拆开：
...
这样，当前设计既不提前选择具体端侧框架，也不会等到 R5 才发现训练特征无法在设备端稳定复现。
...
| **R2 五轴数学领域包** | 接入完备数学模型 | M0–M5、PathProgress、对应/重建策略、模型碰撞验证、参考求解器、证书和测试族 | 通过数学规范 Math F0–F4；面向下游的候选具备完整标准 Claim 且无 `CollisionUnchecked`；无需改写 Core |
...
| **R5 数据集与学习模型** | 形成可信预测能力 | DatasetSnapshot、异常检测、残差或条件效果模型、不确定性、端侧 ModelBundle 候选 | 跨设备和工况验证；域外样本可识别；训练端与目标端结果可核对 |
...
- R2：保持五轴数学规范完整；Math F3 时间与离散参考栈已经通过同一 Core 运行路径落地，覆盖 M4 连续时间参数化、结点停启/dwell、逐轴 V/A/J、M5 固定周期采样和版本化区间重建证据；下一工作面是 F4 Reference Solver/SUT 集成与数学领域总验收。

R3–R7 当前只定义依赖、输出和阶段门，不提前锁定具体驱动器、数据库、训练框架或优化算法。
