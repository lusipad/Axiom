...
近期产品目标收敛为 **CNC 算法对比与回归验证工具**。先接入团队真实算法的两个版本和来源明确的刀路，按同一案例独立检查，生成可定位、可下载、可重放的报告。

当前新增工作流见 [CNC-BENCHMARK.md](CNC-BENCHMARK.md)；它在应用层接入外部算法导出，首先提供采样 XYZ 检查，不要求先完成五轴求解、机器学习或设备部署。

| 顺序 | 交付 | 验收依据 |
|---|---|---|
| 1 | 实际算法 A/B 文件接入 | 相同案例、单位、坐标系与固定周期；可以有不同采样数量和运行时间 |
| 2 | 独立检查与回归报告 | 已知错误被识别，定位到样本、时间和轴；报告注明未检查性质 |
| 3 | 生产刀路与工程师重放 | 真实版本产生一次可解释的比较，另一位工程师独立复现 |
| 4 | 按证据扩展 | 针对真实缺口加入连续重建、计时统计、Orion 或设备观测 |

保留已有数学、单位、谱系、校准/验证分离和现场合同。R5–R7 暂停横向扩展，现有页面作为参考/实验能力保留。下文 R0–R7 是既有能力和历史演进说明，后续阶段不再自动构成开发排期。

Draft PR #21 的多 Trace 新内核、证书和完整有界迁移体系作为后续研究提案；先完成真实算法接入，再根据实际需求决定迁移。当前不启动按其原计划安排的 52–63 周重建。

离线数学与采样回归可独立产生产品价值。真实设备数据用于验证物理预测及现实外推，不作为全部离线检查的前提。具体取舍见 [ADR-0028](架构决策记录/ADR-0028-先交付CNC算法对比与回归工作流.md)。
