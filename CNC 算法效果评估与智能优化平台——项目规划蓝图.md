...
```mermaid
flowchart LR
    Input["有序点 + 可选上下文"] --> Validate["契约与结构检查"]
...
| **R1 有序离散点 MVP** | 完成第一个可运行评估闭环 | 输入、校验、指标、参考比较、带时间 Case、A/B、重放、报告 | 通过领域包规范 §11 的全部条件及 §10.4 一致性样例 |
...
| 阶段 | 面向用户的形态 |
|---|---|
| R0–R1 | Point Lab：导入、检查、评价、比较和生成证据包 |
