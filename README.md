...
> 状态：v0.11.0 Windows 发布版本；R0–R2 已闭合，R3 Windows 只读参考链已闭合，R4.0 synthetic SIL 与 R5-A synthetic learning 合同片已实现，realityValidationStatus / realWorldGeneralizationStatus 保持 Open
> 当前里程碑：通用框架 `R0` + 有序离散点 `R1` + Five-Axis Math `F4` + Machine `R3 v1` + Physical `R4.0 SIL` + Intelligence `R5-A`
> 更新日期：2026-08-12
...
| [数据集与学习模型规范](数据集与学习模型规范.md) | R5 数据集快照、split/governance/lineage、OOD、ModelBundle 和端侧解释器边界 | R5-A / Windows synthetic contract implemented; real generalization open |
...
v0.11.0 保留 Point Lab、Five-Axis F1–F4、Machine Read-only Lab 和 Physical R4，并新增 Intelligence R5-A 工作台。它读取与 Python、HTTP 相同的冻结场景，展示 DatasetSnapshot、分组切分、按轴残差、OOD、区间不确定性、ModelBundle、Claim 与 Evidence；反例只显示预期拒绝，不继承正向模型声明。

v0.11.0 的发布与阻断验收基线是 Windows AMD64、CPython 3.12.10，并固定 `OPENBLAS_CORETYPE=Haswell`、OpenBLAS/OMP 单线程和 [`constraints/acceptance.txt`](constraints/acceptance.txt) 依赖版本。R4/R5 runtime 在非 Windows 环境会明确返回 `UnsupportedRuntimePlatform`，不产生通过结论；Ubuntu/Linux/WSL 不属于本阶段支持矩阵。portable Artifact 身份仍与精确环境绑定的 `RunBundle.bundleHash` 分开验证。
...
v0.11.0 沿用单个评估或 Run 请求最多 8 MiB、单个比较或实验文件最多 16 MiB、单个序列最多 100,000 点、非逐点参考比较最多 5,000,000 个距离单元的预算；当前 Fréchet 实现另有更严格的路径存储预算。F1 名义扫掠差集、F2 连续配置碰撞细分、F3 区间重建和 F4 M5 重建碰撞都有确定性证明边界；超过可证明范围会返回结构化 `Unsupported*` / `Inconclusive`，不会用有限采样伪造正向证书。R3 文件 capture 与 R4 trace 仍受统一 8 MiB CLI/API 请求预算约束，不会静默补齐缺失帧或插值。
...
v0.11.0 的执行边界仍是本地、确定性、静态注册。它不会执行任意命令或 Python 模块，也不启动厂商算法、容器或设备。Point Lab 对三维及更高维数据只显示明确标注的 XY 投影，数值评估仍消费全部坐标；Five-Axis F1–F4 覆盖数学参考链；Machine R3 只读取已落盘的 Windows JSON capture；Physical R4 仅运行 synthetic SIL 一阶轴模型验证；Intelligence R5-A 只重放冻结的合成数据、离线 X-only 训练和目标解释器。数据库、认证、远程队列、文件/RPC Solver Adapter、持久化历史、在线设备协议、真实控制器采集、真实设备数据训练、在线学习、自动部署和参数回写仍属于后续阶段。
