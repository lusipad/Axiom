...
`manifest.json` 由发布测试冻结可跨受支持数值环境重放的 M3/M4/M5 内容身份、重建策略和标准 Claim，并按操作系统、机器架构和完整数值环境分别冻结 `RunBundle` 身份。M3 的 SVD 派生证据保留版本化的 12 位有效数字策略；portable Artifact 哈希使用 8 位有效数字和统一 signed zero 的 JSON 浮点表示。身份归一化不改变任何连续约束、重建或 Claim 门槛使用的原始 binary64 值。任何样本点结论都必须由声明的 reconstruction policy 解释；不得仅凭离散点通过就宣称采样区间安全。
