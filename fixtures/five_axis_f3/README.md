...
`manifest.json` 由发布测试冻结可跨受支持数值环境重放的 M3/M4/M5 内容身份、重建策略和标准 Claim，并按具体数值环境分别冻结完整 RunBundle 身份。M3 的 SVD 派生证据使用版本化的 12 位有效数字规范化策略。任何样本点结论都必须由声明的 reconstruction policy 解释；不得仅凭离散点通过就宣称采样区间安全。
