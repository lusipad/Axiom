...
4. 导入后的 `captureReceipt.operation` 保持 `file-import`，因为 Axiom 消费的是已封存 dossier 文件；`sourceKind` 保持 `device-read`，表示数据的原始事实来源。`traceContentHash` 绑定原始 validation Shadow evidence 的内容哈希；新生成的 Observation、PhysicalResponseTrace 和 CaseEvidence 分别生成自己的内容哈希，并由 `RealHoldoutProjectionReceipt` 同时记录源与目标身份。
